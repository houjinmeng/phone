<template>
  <div style="text-align:center">
    ...
    <!-- 列表区域 -->
    <ul id="liststyle">
      <li id="box" v-for="(item,k) in tableData" :key="k" @click="look(item.id)">
  ...
  methods: {
    ...
    look(uid) {
      const data = {
        ...
        check_id: uid.check_id,
        machine_id: uid.machine_id
      };
      this.$http.post("/play_detail", JSON.stringify(data)).then(res => {
        this.$router.push({
          name: "statisticsItem",
          params: {
            message: res.data
