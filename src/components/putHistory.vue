    ...
    <ul id="liststyle">
      <li id="box" v-for="(item,k) in tableData" :key="k" @click="look(item)">
        ...
        <ul class="left" style="margin-right:">
          <li style="font-size:0.16rem;margin-top:25%;width:0.8rem;height:0.18rem;overflow:hidden">{{item.machine_name.join(';')}}</li>
        </ul>
        <ul class="left">
          <li style="margin:0.05rem 0 0 0.3rem;width:1.8rem;height:0.18rem;overflow:hidden">{{item.address.join(';')}}</li>
          <li
